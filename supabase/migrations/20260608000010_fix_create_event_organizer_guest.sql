-- The birthday migration (20260608000000) replaced create_event using an inline
-- RETURN QUERY INSERT pattern, which dropped the organizer auto-guest insertion
-- that 20260605030000 had added. Restore it and backfill affected events.
...
-- Backfill organizer guest rows for any events missing them.
INSERT INTO event_guests (event_id, user_id, display_name, status, role)
SELECT
  e.id,
  e.created_by,
